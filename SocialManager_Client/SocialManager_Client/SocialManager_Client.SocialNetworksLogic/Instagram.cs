...
namespace SocialManager_Client.SocialNetworksLogic
{
    class Instagram
    {
        ...
        /// <summary>
        /// Get users matching with query
        /// </summary>
        /// <param name="count">Number of results. Default 10</param>
        /// <returns></returns>
        public List<IMinifiedUser> SearchUsers(string q, int count = 10)
        {
            try
            {
                return SearchManager.GetUsers(new SearchUserParameters()
                {
                    Count = count,
                    Query = q
                });
        ...
        /// <summary>
        /// Gives info of relation ship between authenticated user and given one
        /// </summary>
        public IRelationship GetFriendShipWith(long userId)
        {
            try
            {
                return RelationshipsManager.GetRelationshipinfo(userId);
            }
            catch (InstagramAPICallException)
            {
                return null;
